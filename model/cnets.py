...
class Model(nn.Module):
    ...
    def topK_genrate(
        ...
        last_headout = self.lm_head(self.norm(last_hidden)) #torch.Size([1, 32000])
        ...
        scores = topk_p[0] #torch.Size([10])
        scores_list.append(scores[None])
        parents_list.append(torch.zeros(1, dtype=torch.long, device=scores.device)) #0
        ...
        tree_mask = self.tree_mask_init #torch.Size([1, 1, 10, 10])
        ...
        for i in range(depth):
            ...
            last_headout = self.lm_head(self.norm(out_hidden[0])) #torch.Size([10, 32000])
            last_p = self.logsoftmax(last_headout)

            top = torch.topk(last_p, top_k, dim=-1) #torch.Size([10, 10])
            topk_index, topk_p = top.indices, top.values
        ...
        scores_list = torch.cat(scores_list, dim=0).view(-1) #torch.Size([610])
        ss_token_list = torch.cat(ss_token, dim=0).view(-1)

        
        top_scores = torch.topk(scores_list, total_tokens, dim=-1) #torch.Size([31])
        top_scores_index = top_scores.indices
        top_scores_index = torch.sort(top_scores_index).values

        
        draft_tokens = ss_token_list[top_scores_index] #torch.Size([32])
        draft_tokens = torch.cat((sample_token, draft_tokens), dim=0)

        
        final_path_scores = scores_list[top_scores_index]  
        
        node_logprob = torch.cat([torch.zeros(1, device=final_path_scores.device), final_path_scores], dim=0)  # [S]
